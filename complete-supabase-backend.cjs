...
const emailTransporter = nodemailer.createTransporter({
  service: 'gmail',
  auth: {
    user: process.env.EMAIL_USER || 'farmtally.notifications@gmail.com',
...
app.post('/system-admin/users/:userId/reject', async (req, res) => {
  ...
  try {
    ...
    const rejectionEmailHtml = `
      <h2>Account Registration Update - FarmTally</h2>
      <p>Dear ${user.profile?.firstName || 'User'},</p>
      <p>Thank you for your interest in FarmTally. Unfortunately, we cannot approve your account registration at this time.</p>
      ${rejectionReason ? `<p><strong>Reason:</strong> ${rejectionReason}</p>` : ''}
      <p>If you have any questions, please contact our support team.</p>
      <p>Thank you for your understanding.</p>
    `;

    await sendEmail(user.email, 'Account Registration Update - FarmTally', rejectionEmailHtml);
...
app.listen(PORT, () => {
  console.log(`FarmTally Backend with Supabase running on port ${PORT}`);
});