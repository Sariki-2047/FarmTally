...
const lorry_service_1 = require("../services/lorry.service");
class LorryController {
    lorryService;
    constructor() {
        this.lorryService = new lorry_service_1.LorryService();
    }
    createLorry = async (req, res, next) => {
        try {
    ...
    getLorries = async (req, res, next) => {
        try {
            ...
            const result = await this.lorryService.getLorries(organizationId, userId, filters, parseInt(page), parseInt(limit));
            res.json({
                success: true,
    ...
    getLorryById = async (req, res, next) => {
        try {
    ...
    updateLorry = async (req, res, next) => {
        try {
            const { lorryId } = req.params;
            const userId = req.user.id;
    ...
    deleteLorry = async (req, res, next) => {
        try {
            const { lorryId } = req.params;
            const userId = req.user.id;
    ...
    assignLorryToManager = async (req, res, next) => {
        try {
            const { lorryId } = req.params;
    ...
    unassignLorry = async (req, res, next) => {
        try {
            const { lorryId } = req.params;
            const userId = req.user.id;
    ...
    getLorryStats = async (req, res, next) => {
        try {
            const { organizationId } = req.params;
            const userId = req.user.id;
...
exports.LorryController = LorryController;
//# sourceMappingURL=lorry.controller.js.map